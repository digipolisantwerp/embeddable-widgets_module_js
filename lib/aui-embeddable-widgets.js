...
const reactComponent = (widgetUrl, deps, overrides) => class extends deps.React.Component {
  ...
  componentDidMount() {
    (0, _widgets.load)(widgetUrl, overrides).then(widget => {
      // convert widget into react component
      const component = widget.component.driver('react', deps); // monkey-patch zoid's react component to make it respect className
...
const {
  create
} = zoid; // registered widgets, indexed by tag

const widgets = {}; // maps url to promise of widget definition

const fetchedUrls = {};

const scrollTo = (elementOffset, tag) => {
  const containerElement = document.querySelector("[id^='zoid-".concat(tag, "-']"));
...
function xhrGet(url) {
  return new _zalgoPromise.ZalgoPromise((resolve, reject) => {
...
function define(definition) {
  ...
  if (widgets[tag]) {
    ...
    const componentDefinition = (0, _deepmerge.default)(widgetDefaults, definition); // convert from JSON to zoid syntax
...
function load(url, overrides, force) {
  ...
  const loaded = fetchedUrls[url]; // don't load if already loading or loaded, unless forced

  if (!loaded || force) {
    // inherit overrides from parent if they exist
    const allOverrides = _extends({}, getParentOverrides(), overrides); // start loading and cache the promise


    fetchedUrls[url] = xhrGet(url).then(response => {
      const defaultDefinition = _extends(JSON.parse(response), allOverrides, {
        originalUrl: url
      });
...
function render(tag, props, elem) {
  ...
  const def = widget.componentDefinition; // pass overrides from parent to child

  props._aui_overrides = widget.overrides;
  const instance = widget.component(props); // There is an off-by-one bug in the height calculations because zoid uses offsetHeight
