...
function render(tag, props, elem) {
  ...
  var cmp = widget.component(props); // There is an off-by-one bug in the height calculations because zoid uses offsetHeight
  ...
  if (def.autoResize && def.autoResize.height) {
    var lastHeight = null;
    cmp.event.on('zoid-resize', function (_ref) {
      var height = _ref.height;

      // if we're not just responding to our own height change
      if (height && lastHeight !== height) {
        var newHeight = height + 1;
        lastHeight = newHeight; // make sure we do this AFTER zoid updates the height
